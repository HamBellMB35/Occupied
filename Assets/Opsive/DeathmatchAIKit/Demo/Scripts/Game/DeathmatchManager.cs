...
namespace Opsive.DeathmatchAIKit.Demo.Game
{
    ...
    public class DeathmatchManager : MonoBehaviour
    {
        ...
        private static DeathmatchManager Instance
        {
            get {
                if (s_Instance == null) {
                    s_Instance = new GameObject("DeathmatchManager").AddComponent<DeathmatchManager>();
                }
                return s_Instance;
            }
        }
        ...
        private bool PausedInternal
        {
            set {
                ...
                if (m_LocalPlayer == null || m_Paused == value) {
                    return;
                }
                m_Paused = value;
                ...
                EventHandler.ExecuteEvent(m_LocalPlayer, "OnEnableGameplayInput", !m_Paused);
        ...
        public void SceneLoaded()
        {
            ...
            var characterMonitors = FindObjectsOfType<CharacterMonitor>(true);
        ...
        private GameObject InstantiatePlayer(GameObject prefab, string name, int teamIndex)
        {
            ...
            player.name = name;
            // Update the child model name as well.
            player.GetComponentInChildren<AnimatorMonitor>().name = name;
        ...
        private void SetupPlayer(GameObject player, int playerIndex, int teamIndex, int friendlyLayer, int enemyLayer, Color primaryColor, Color secondaryColor)
        {
            ...
            if (m_ObserverMode || playerIndex != 0) {
                ...
                if (teamIndex > 0) {
                    ...
                    // Hard: No change.
                    ShootableAction[] shootableActions;
                    var spread = -1f;
                    switch (m_Difficulty) {
                    ...
                    if (spread != -1) {
                        shootableActions = deathmatchAgent.GetComponentsInChildren<ShootableAction>();
                        for (int i = 0; i < shootableActions.Length; ++i) {
                            var shooterModules = shootableActions[i].ShooterModuleGroup;
                            for (int j = 0; j < shooterModules.ModuleCount; ++j) {
                                var shooterModule = shooterModules.GetModuleAt(j);
                                if (shooterModule is UltimateCharacterController.Items.Actions.Modules.Shootable.HitscanShooter) {
                                    var hitscanShooter = shooterModule as UltimateCharacterController.Items.Actions.Modules.Shootable.HitscanShooter;
                                    hitscanShooter.Spread = spread;
                                } else if (shooterModule is UltimateCharacterController.Items.Actions.Modules.Shootable.ProjectileShooter) {
                                    var projectileShooter = shooterModule as UltimateCharacterController.Items.Actions.Modules.Shootable.ProjectileShooter;
                                    projectileShooter.Spread = spread;
                                }
                            }
                        }
                    }
