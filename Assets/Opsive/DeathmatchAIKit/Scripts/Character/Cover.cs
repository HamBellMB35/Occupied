...
namespace Opsive.DeathmatchAIKit.Character.Abilities
{
    ...
    public class Cover : DetectObjectAbilityBase
    {
        ...
        protected override void AbilityStarted()
        {
            ...
            var direction = moveTowardsLocation != null ? (moveTowardsLocation.TargetRotation * Vector3.forward) : (m_Rigidbody.rotation * Vector3.forward);
            var radius = m_CharacterLocomotion.Radius / 2;
            var collision = Physics.SphereCast(m_Rigidbody.position + m_CharacterLocomotion.Center - direction * radius, radius,
            ...
            var characterPosition = m_Rigidbody.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height;
            var position = m_Rigidbody.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height + Quaternion.LookRotation(-m_RaycastResult.normal) * new Vector3(m_ExposedDistance.x, 0, 0);
            ...
            if (m_CanTakeCrouchingCover && !m_CharacterLocomotion.IsAbilityTypeActive<HeightChange>()) {
                position = m_Rigidbody.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height;
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if ((startingAbility is Aim) && !m_AtEdge) {
                ...
                if (!m_CharacterLocomotion.IsAbilityTypeActive<HeightChange>() && !m_Standing) {
                    var position = m_Rigidbody.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height;
        ...
        public override void Update()
        {
            ...
            var collision = Physics.SphereCast(m_Rigidbody.position + m_CharacterLocomotion.Center + m_RaycastResult.normal * radius,
            ...
            if (m_DebugExposedDistanceRay) {
                ...
                var debugPosition = m_Rigidbody.position + m_CharacterLocomotion.Center + m_RaycastResult.normal * m_RaycastResult.distance +
            ...
            if (m_CanTakeCrouchingCover && !m_CharacterLocomotion.IsAbilityTypeActive<HeightChange>() && m_LastStandingSwitch + m_MinAutoCoverSwitchDuration < Time.time) {
                var position = m_Rigidbody.position + m_CharacterLocomotion.Up * m_CharacterLocomotion.Height + m_RaycastResult.normal * radius;
        ...
        private void CheckForEdge(float horizontalMovement)
        {
            ...
            var raycastRotation = Quaternion.LookRotation(m_RaycastResult.normal, m_CharacterLocomotion.Up);
            var raycastPosition = m_Rigidbody.position + m_CharacterLocomotion.Center + m_RaycastResult.normal * m_RaycastResult.distance +
        ...
        public override void UpdateRotation()
        {
            ...
            var targetRotation = Quaternion.RotateTowards(m_Rigidbody.rotation, Quaternion.LookRotation(raycastNormal, m_CharacterLocomotion.Up), m_CharacterLocomotion.MotorRotationSpeed);
        ...
        public override void UpdatePosition()
        {
            ...
            targetPosition = m_Rigidbody.TransformPoint(localTargetPosition);
            var deltaPosition = Vector3.MoveTowards(m_Rigidbody.position, targetPosition, m_MoveSpeed) - m_Rigidbody.position;
            m_CharacterLocomotion.DesiredMovement = deltaPosition / (Time.timeScale * TimeUtility.FramerateDeltaTime);
        }
        ...
        public override void ApplyPosition()
        {
            ...
            if (m_AtEdge && m_RestrictPosition) {
                var localMoveDirection = m_Rigidbody.InverseTransformDirection(m_CharacterLocomotion.DesiredMovement);
                if ((m_LookRight && localMoveDirection.x > 0) || (!m_LookRight && localMoveDirection.x < 0)) {
                    localMoveDirection.x = 0;
                    m_CharacterLocomotion.DesiredMovement = m_Rigidbody.TransformDirection(localMoveDirection);
                }
            }

            // Prevent the character from moving in front of the cover position.
            var localCoverDirection = MathUtility.InverseTransformPoint(m_RaycastResult.point, Quaternion.LookRotation(m_RaycastResult.normal, m_CharacterLocomotion.Up), m_Transform.position + m_CharacterLocomotion.DesiredMovement);
