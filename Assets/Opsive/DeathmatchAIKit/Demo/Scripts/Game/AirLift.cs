...
namespace Opsive.DeathmatchAIKit.Demo.Game
{
    ...
    public class AirLift : MonoBehaviour
    {
        ...
        [SerializeField] protected LayerMask m_CharacterLayer = 1 << UltimateCharacterController.Game.LayerManager.Character | 1 << UltimateCharacterController.Game.LayerManager.Enemy;
        [Tooltip("The upwards force to apply to each character.")]
        [SerializeField] protected float m_UpwardsForce = 0.9f;
        [Tooltip("The forwards force to apply to each character, in the direction of the airlift's forward transform.")]
        [SerializeField] protected float m_ForwardsForce = 9f;
        ...
        {
            if (!UltimateCharacterController.Utility.MathUtility.InLayerMask(other.gameObject.layer, m_CharacterLayer)) {
                return;
            }
