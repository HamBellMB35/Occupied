...
namespace Opsive.DeathmatchAIKit.Demo.UI
{
    ...
    {
        [Tooltip("The Item Collection used by the monitor editor.")]
        ...
        {
            // The character may be set before the component is initialized.
            if (m_WheelSlices == null) {
                Awake();
            }
            ...
            if (character != null) {
                ...
                var itemSetGroups = itemSetManager.ItemSetGroups[categoryIndex];

                for (int i = 0; i < m_WheelSlices.Length; ++i) {
                    m_WheelSlices[i].Initialize(character);
                }

                EventHandler.RegisterEvent<ItemSetGroup>(character, "OnItemSetGroupUpdated", ItemSetGroupUpdated);
                m_PlayerInput = character.GetCachedComponent<IPlayerInput>();
                enabled = true;
            } else if (m_Character != null) {
                EventHandler.UnregisterEvent<ItemSetGroup>(m_Character, "OnItemSetGroupUpdated", ItemSetGroupUpdated);
            }
        ...
        /// <summary>
        /// The ItemSet Group has updated.
        /// </summary>
        /// <param name="group">The group that updated.</param>
        private void ItemSetGroupUpdated(ItemSetGroup group)
        {
            for (int i = 0; i < group.ItemSetList.Count; ++i) {
                var itemSet = group.ItemSetList[i];
                for (int j = 0; j < itemSet.SlotCount; ++j) {
                    if (itemSet.ItemIdentifiers[j] == null) {
                        continue;
                    }

                    var itemDefinition = itemSet.ItemIdentifiers[j].GetItemDefinition();
                    if (!m_ItemDefinitionByItemIndex.ContainsKey(itemDefinition)) {
                        m_ItemDefinitionByItemIndex.Add(itemDefinition, i);
                    } else {
                        m_ItemDefinitionByItemIndex[itemDefinition] = i;
                    }
                }
            }
        }
        ...
        public void ItemSelected(ItemDefinitionBase itemDefinition)
        {
            if (!m_ItemDefinitionByItemIndex.TryGetValue(itemDefinition, out var index)) {
                Debug.Log("Cannot find " + itemDefinition);
                return;
            }
