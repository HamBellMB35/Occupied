...
namespace Opsive.DeathmatchAIKit.Demo.UI
{
    ...
    public class FullScoreboardMonitor : MonoBehaviour
    {
        ...
        protected class ScoreContainer
        {
            ...
            [SerializeField] protected Shared.UI.Text m_Name;
            [Tooltip("The number of player kills.")]
            [SerializeField] protected Shared.UI.Text m_Kills;
            [Tooltip("The number of player deaths.")]
            [SerializeField] protected Shared.UI.Text m_Deaths;
            ...
            public Shared.UI.Text Name { get { return m_Name; } }
            public Shared.UI.Text Kills { get { return m_Kills; } }
            public Shared.UI.Text Deaths { get { return m_Deaths; } }
        ...
        private void ShowHideScore(bool show, bool endScreenVisible)
        {
            ...
            if (show) {
                ...
                for (int i = 0; i < teamStats.Count; ++i) {
                    ...
                    if (DeathmatchManager.TeamGame) {
                        m_Teams[i].Name.SetText(teamStats[i].Name);
                        m_Teams[i].Kills.SetText(teamStats[i].Kills.ToString());
                    ...
                    for (int j = 0; j < playerStats.Count; ++j) {
                        playerText[playerCount + j].Name.SetText(playerStats[j].Player.name);
                        playerText[playerCount + j].Kills.SetText(playerStats[j].Kills.ToString());
                        playerText[playerCount + j].Deaths.SetText(playerStats[j].Deaths.ToString());
