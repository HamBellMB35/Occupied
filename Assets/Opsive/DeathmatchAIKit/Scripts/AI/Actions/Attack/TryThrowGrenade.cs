...
namespace Opsive.DeathmatchAIKit.AI.Actions
{
    ...
    public class TryThrowGrenade : StartStopUse
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            var item = m_InventoryBase.GetActiveCharacterItem(m_SlotID.Value);
            ...
            for (int i = 0; i < m_InventoryBase.SlotCount; ++i) {
                ...
                var equippedItem = m_InventoryBase.GetActiveCharacterItem(m_SlotID.Value);
                ...
                if (equippingItem != null && m_DeathmatchAgent.WeaponStatForItemDefinition(equippingItem.GetItemDefinition()).Class == DeathmatchAgent.WeaponStat.WeaponClass.Power) {
                    return TaskStatus.Failure;
                }
            }

            // Adjust the throwable velocity.
            var throwableAction = weaponStat.UsableAction as ThrowableAction;
            if (throwableAction != null) {
                var projectileThrower = throwableAction.ThrowerModuleGroup.FirstEnabledModule as UltimateCharacterController.Items.Actions.Modules.Throwable.ProjectileThrower;
                if (projectileThrower != null) {
                    var velocity = projectileThrower.Velocity;
                    velocity.z = Vector3.Distance(targetParent.position, transform.position) * m_GrenadeForceMultiplier.Value;
                    projectileThrower.Velocity = velocity;
                }
            }
        ...
        {
            var item = usableItem.CharacterItem;
            var weaponStat = m_DeathmatchAgent.WeaponStatForItemDefinition(item.ItemDefinition);
