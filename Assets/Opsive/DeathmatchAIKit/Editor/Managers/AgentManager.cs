...
namespace Opsive.DeathmatchAIKit.Editor.Managers
{
    ...
    public class AgentManager : Manager
    {
        ...
        private void BuildAgent()
        {
            ...
            var lookTransform = m_Agent.GetComponentInChildren<AnimatorMonitor>(true).GetComponent<Animator>().GetBoneTransform(HumanBodyBones.Head);
            ...
            var items = m_Agent.GetComponentsInChildren<CharacterItem>();
            ...
            for (int i = 0; i < items.Length; ++i) {
                ...
                var itemAction = items[i].GetComponent<CharacterItemAction>();
                if (typeof(MeleeAction).IsAssignableFrom(itemAction.GetType())) {
                    weaponStat.Class = DeathmatchAgent.WeaponStat.WeaponClass.Melee;
                } else if (typeof(ThrowableAction).IsAssignableFrom(itemAction.GetType())) {
            ...
            var animator = m_Agent.GetComponentInChildren<AnimatorMonitor>(true).GetComponent<Animator>();
